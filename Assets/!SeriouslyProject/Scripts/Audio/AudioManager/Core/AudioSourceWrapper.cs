...
namespace AudioManager.Core
{
    ...
    public class AudioSourceWrapper
    {
        ...
        public void DeregisterChildren()
        {
            ...
            {
                // Добавлена проверка на null перед уничтожением
                if (childSource != null)
        ...
        {
            // ИЗМЕНЕНИЕ: Используем '==' вместо 'is null'. 
            // 'is null' проверяет только C# ссылку (которая существует), 
            // а '==' использует перегрузку Unity для проверки уничтоженного нативного объекта.
            if (m_wrappedSource == null)
            {
                return;
            }
        ...
        {
            // ИЗМЕНЕНИЕ: Используем '==' вместо 'is null' для корректной проверки уничтоженных Unity объектов.
            if (m_wrappedSource == null)
            {
                return default;
            }
        ...
        {
            // ИЗМЕНЕНИЕ: Используем '==' вместо 'is null'. 
            // Это предотвращает вызов 'get.Invoke', который вызывает ошибку MissingReferenceException.
            if (m_wrappedSource == null)
            {
                return default;
            }
        ...
        public void SetChildrenVolume(float volume)
        {
            ...
            {
                // Unity перегружает operator !=, так что это безопасно
                if (child != null)
                {
                    child.volume = volume;
                }
