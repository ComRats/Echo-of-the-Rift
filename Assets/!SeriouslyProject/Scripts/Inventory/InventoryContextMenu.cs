...
public class InventoryContextMenu : MonoBehaviour
{
    ...
    private void CreateButtonsForItem(DraggableItem item)
    {
        ...
        // Кнопка "Информация" для всех предметов
        CreateButton("Информация", () => ShowItemInfo(item));
    }
    ...
    private void ShowItemInfo(DraggableItem item)
    {
        Debug.Log($"=== Информация о предмете ===");
        Debug.Log($"Название: {item.itemData.itemName}");
        Debug.Log($"Игровое название: {item.itemData.itemGameName}");
        Debug.Log($"Тип: {item.itemData.itemType}");
        Debug.Log($"Количество: {item.count}");
        Debug.Log($"Складируемый: {item.itemData.isStackable}");
        
        // Здесь можно открыть UI панель с подробной информацией
    }
